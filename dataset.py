...
from skimage.morphology import black_tophat, convex_hull_image, skeletonize, square, disk, dilation, erosion
...
def saw_distmap_parallel(g):
    ...
    skeleton_union, contour_union = contour_skeleton.apply(lambda x: x[0]).sum(), contour_skeleton.apply(
        lambda x: x[1]).sum()
    ...
    weight = _calc_weight_class(g, eps=eps) + ndi.gaussian_filter(contour_union * (1 - phi_k / (tau + eps)),
                                                                  sigma=sigma)
...
def load_data(root_path,
    ...
    thresh_option : string
        Threshold option for binarizing output mask predictions
    ...
    assert return_dist is False or \
           return_dist is None or \
